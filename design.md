...
页面以真实工作阶段组织内容。应用侧栏仍是全局导航，页面内部使用双目录 Mega Menu：桌面端按 5:4 分为「业务作业 SOP」与「系统使用 SOP」，移动端按同一阅读顺序上下排列。业务作业沿用物料准备、现场工作、后续交接；系统指引对应入口、操作、核对，并提供真实功能页面链接。

系统指引按现有角色权限向下兼容：使用人看到基础操作；审批人额外看到样机审批；管理员看到审批和管理指引；超级管理员看到全部内容。Liquid 只承载导航和阶段切换，不用于正文装饰；图文提示以 Lucide 图标和简洁路径说明呈现，真实按钮、文字、焦点环与点击区域保持清晰。机场选址与安装部署使用已确认的业务内容；未确认的维修流程显示待配置空状态，不生成假流程。
