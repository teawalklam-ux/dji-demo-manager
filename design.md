...
第一屏先展示真实库存统计，不制造任何指标。页面标题与快捷操作组成同一工作头，四项核心统计使用等宽独立数据卡，按桌面四列、平板两列、移动单列重排；预警、图表、审批与变动记录使用 7/5 与 5/7 的非对称单层工作面。无数据时保留解释与下一步操作，不再让区块直接消失。
...
### SOP 指引：Workbench Guided Sequence / Narrative Workflow / Sliding Tabs
...
- 默认微交互只有按钮按压与箭头交接、统计卡轻微位移、指标逐位上滚、状态圆盘圆角环段绘制与当前环段轻微放大，以及侧栏和 SOP 当前项指示轨的位置过渡。按钮与页面等待统一使用 DJI 蓝 Halo，真实图片等待使用 Scan 扫描占位；两者只表达加载状态，不用于装饰。指标在首次显示和精细指针再次移入时重播一次；圆盘只在首次显示时绘制，不循环播放。状态环段悬停、图例聚焦或轻触时，中心数值同步切换。
- 功能性弹窗与抽屉只使用短促的透明度或位移反馈，不引入装饰性弹跳。
- SOP 页面只允许活动底板横向滑动、内容短距离交接和截图工具栏显隐；固定几何不参与插值，动效关闭时直接显示最终状态。
...
- 基础布局从 320 px 开始。
- 统计、工具头与筛选条件允许容器重排，按钮文字不换行；仪表盘次级快捷入口在移动端使用两列网格，样机列表筛选框保持紧凑双列，不拉伸为整行长条。
- 数据表在小屏继续切换为现有移动卡片。
...
@theme {
  ...
  --dur-loader-halo: 1200ms;
  --dur-loader-scan: 2350ms;
}
```
...
```json
{
  ...
  "duration": {
    ...
    "loader-halo": { "$value": "1200ms", "$type": "duration" },
    "loader-scan": { "$value": "2350ms", "$type": "duration" }
  },
  "radius": {
