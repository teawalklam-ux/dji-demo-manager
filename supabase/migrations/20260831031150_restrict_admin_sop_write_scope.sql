-- Keep persistence writes within the same role boundary used for reads.
drop policy if exists "管理员可新增SOP" on public.sop_processes;
...
-- Avoid the PostgreSQL CURRENT_ROLE keyword when resolving the application
-- profile role inside the invoker function.
