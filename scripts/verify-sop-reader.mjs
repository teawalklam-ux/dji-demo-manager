...
const out = resolve(process.env.SOP_CAPTURE_OUTPUT || 'artifacts/sop-reader')
...
const imageViewer = (page) => page.locator('.sop-image-viewer')
...
try {
  ...
  for (const width of [320, 375, 414, 768, 1280, 1440, 1920]) {
    ...
    const viewerTrigger = dialog(page).getByRole('button', { name: '放大截图', exact: true })
    await viewerTrigger.click()
    await imageViewer(page).waitFor({ state: 'visible' })
    await page.waitForTimeout(220)
    if (width === 320 || width === 1440) await page.screenshot({ path: resolve(out, `viewer-${width}.png`), animations: 'disabled' })
    const viewerBounds = await imageViewer(page).boundingBox()
    const viewerImageBounds = await imageViewer(page).locator('img').boundingBox()
    const viewerControlsBounds = await imageViewer(page).locator('.sop-image-viewer__controls').boundingBox()
    const edgeSpace = width < 640 ? 32 : 72
    assert(viewerBounds.x === 0 && viewerBounds.y === 0 && Math.abs(viewerBounds.width - width) <= 1 && Math.abs(viewerBounds.height - 900) <= 1, `viewer fills ${width}px viewport: ${JSON.stringify(viewerBounds)}`)
    assert(viewerImageBounds.x >= -1 && viewerImageBounds.y >= -1 && viewerImageBounds.x + viewerImageBounds.width <= width + 1 && viewerImageBounds.y + viewerImageBounds.height <= 901, `fit image is visible at ${width}`)
    assert(Math.abs(viewerImageBounds.width - (width - edgeSpace)) <= 2 || Math.abs(viewerImageBounds.height - (900 - edgeSpace)) <= 2, `one image edge fits viewport at ${width}: ${JSON.stringify(viewerImageBounds)}`)
    assert(viewerControlsBounds.x >= 0 && viewerControlsBounds.x + viewerControlsBounds.width <= width, `viewer controls fit ${width}`)
    const initialZoom = Number((await imageViewer(page).locator('output').innerText()).replace('%', ''))
    const zoomIn = imageViewer(page).getByRole('button', { name: /放大图片/ })
    await zoomIn.click()
    assert.equal(Number((await imageViewer(page).locator('output').innerText()).replace('%', '')), initialZoom + 5, 'tap zoom uses 5% step')
    const zoomInBounds = await zoomIn.boundingBox()
    await page.mouse.move(zoomInBounds.x + zoomInBounds.width / 2, zoomInBounds.y + zoomInBounds.height / 2)
    await page.mouse.down()
    await page.waitForTimeout(300)
    await page.mouse.move(zoomInBounds.x + zoomInBounds.width / 2 + 32, zoomInBounds.y + zoomInBounds.height / 2)
    await page.mouse.up()
    assert(Number((await imageViewer(page).locator('output').innerText()).replace('%', '')) > initialZoom + 5, 'hold and sweep changes zoom continuously')
    assert(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), 'fullscreen viewer does not expand the document')
    await imageViewer(page).getByRole('button', { name: '退出图片查看', exact: true }).click()
    await imageViewer(page).waitFor({ state: 'hidden' })
    assert(await viewerTrigger.evaluate((node) => node === document.activeElement), 'closing viewer restores focus to its trigger')
    await dialog(page).getByRole('button', { name: '下一步', exact: true }).click()
    await readyImage(page)
    assert(await dialog(page).getByRole('button', { name: '放大截图', exact: true }).isVisible(), 'viewer trigger remains available after paging')
    ...
    results.push(`${width}px: modal bounds, fixed next, fullscreen fit viewer, tap/sweep zoom, close, paging and reduced motion`)
    await context.close()
  }
